...
class UserEstablishmentImpl(UserEstablishmentRepository):
    ...
    def remove_user(self, user_id: int, establishment_id: int):
        UserEstablishment.objects.filter(user_id=user_id, establishment_id=establishment_id).update(is_active=False)
        
    def remove_all_users(self, establishment_id: int):
        UserEstablishment.objects.filter(establishment_id=establishment_id).update(is_active=False)
