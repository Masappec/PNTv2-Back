...
def auth_send_password_reset_event(current_user_id, username, email, reset_password_url):
    ...
    context['reset_password_url'] = FRONTEND_PASSWORD_CONFIRMATION_URL.replace(':token', context['reset_password_url'])
    

    service = SmtpService(smtp_repository=StmpImpl())
    mail = service.send_email( email, 'Password Reset', 'Password Reset', '', '', '', current_user_id)
    service.send_email_with_template_and_context(mail,'emails/password-reset/password-reset.html', context)
...
def auth_send_activate_account_event(email, uidb64, token,username):
    ...
    context = data['payload']
    context['activate_account_url'] = FRONTEND_ACTIVATE_ACCOUNT_URL.replace(':token', token).replace(':uidb64', uidb64)
    user_id = force_str(urlsafe_base64_decode(uidb64))


    service = SmtpService(smtp_repository=StmpImpl())
    mail = service.send_email( email, 'Activate Account', 'Activate Account', '', '', '', user_id)
    service.send_email_with_template_and_context(mail,'emails/activate-account/activate-account.html', context)
